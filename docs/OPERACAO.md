...
| `MINIO_BUCKET_VIDEOS` | sim | Nome do bucket de vídeos |
| `AUTH_JWT_SECRET` | sim | Segredo simétrico que assina e verifica o JWT próprio da aplicação (SDD § D-03). Ignora qualquer provedor externo — jamais no cliente |
...
`apps/admin` (modelo em [`apps/admin/.env.example`](../apps/admin/.env.example)) — o painel não tem nenhuma variável obrigatória: login é via `POST /api/auth/login`, na própria API, sem provedor externo (SDD § D-03):
...
**O painel precisa da API no ar** para fazer qualquer coisa, inclusive autenticar: ele lê e grava conteúdo, mídia e leads sempre pela API, nunca direto no MySQL ou no MinIO (SDD § "Camadas e padrão arquitetural"). Como painel e API respondem no mesmo endereço, o painel chama caminhos relativos e não existe requisição entre origens a liberar — é o mesmo desenho do domínio único de produção. Com a API fora do ar, o painel entra normalmente e avisa na tela que não conseguiu falar com ela.

A LP não depende da API para renderizar (SDD § D-08): com a API fora do ar, `/` continua servindo a página.

As migrações **já foram aplicadas** contra o MySQL do compose (o registro de execução está em [`../agent_context/PLAN.md`](../agent_context/PLAN.md)). A carga inicial de conteúdo já foi executada: o banco tem as seções, as mídias e os metadados publicados.
...
- **Verificação do schema físico do banco:** `npm run verify:schema -w apps/api` (ver [BANCO-DE-DADOS.md, "Verificar o schema físico contra o SDD"](BANCO-DE-DADOS.md)). Não entra no `npm run test` porque precisa de um MySQL alcançável e de credenciais — é um passo de verificação de ambiente, não um teste unitário.
...
- **Formato de erro:** toda rota que falha responde `{ statusCode, error, fields? }`, e nada além disso — `fields` mapeia o caminho do campo (`hero.headline`) para a mensagem em português. A mensagem é escolhida a partir do status, nunca copiada da exceção, para que caminho de arquivo, nome de variável de ambiente ou detalhe de infraestrutura fiquem no log do servidor e não na resposta. Erro de validação responde `422`, como o SDD determina, e não o `400` padrão do NestJS.
- **Autenticação da API:** o painel autentica em `POST /api/auth/login`, na própria API, e manda o token recebido em `Authorization: Bearer <token>`; a API verifica a assinatura com o segredo simétrico `AUTH_JWT_SECRET` (HS256, SDD § D-03) — não há mais provedor externo nem JWKS. A guarda é **global e nega por padrão**: um endpoint novo, criado sem nenhuma marcação, nasce protegido, e só fica público se alguém escrever `@Public()` nele de propósito — esquecer leva a "bloqueado", nunca a "exposto". Toda recusa sai como `401` no formato único de erro, sem distinguir token ausente de expirado ou de assinatura inválida, para não virar oráculo de tokens válidos; o motivo fica no log do servidor, em texto fixo que nunca inclui o token nem o segredo. Os testes de autenticação não tocam a rede: assinam os próprios tokens localmente com um `AUTH_JWT_SECRET` de teste (`jose`, HS256) — não há par de chaves nem servidor JWKS a simular.
...
- **Proteção de rota do painel:** os testes do painel não tocam a rede — o dublê entra no lugar do `fetch` usado por `ApiAuthGateway`, e adaptador, provedor, guarda, roteador e telas exercitados são os de produção. A guarda é provada por mutação, não por leitura: removê-la derruba as 9 provas de rota e sessão, e removê-la **apenas** no estado `verificando` — o caso em que o painel piscaria conteúdo protegido e só depois redirigiria — ainda derruba 3, porque três testes registram cada nó inserido no documento e falham se a área administrativa chegou a existir, ainda que por um quadro. Uma verificação feita depois de a tela assentar não pegaria isso.
