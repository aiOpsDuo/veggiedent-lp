...
**Achado adicional na mesma verificação, sem relação com os 6 commits:** ao ler as migrações reais de `supabase/migrations/` para preparar a tarefa `migracao-mysql/persistencia-orm`, `agent_context/SDD.md` § "Modelo de dados" descrevia `site_metadata` com uma coluna `canonical_url` **removida desde 2026-09-05** (T25) e sem citar `og_image_alt`, existente desde 2026-09-02 — uma divergência entre documento e código que já existia antes desta sessão, não introduzida por ela. Corrigida a descrição da tabela no SDD, conferida contra `supabase-site-metadata.repository.ts` (a fonte real de quais colunas a aplicação lê e grava). Nenhum código precisa mudar por causa disso — só a descrição estava errada.

## 2026-09-21 — Logo do painel ainda depende do Supabase, achado na varredura final da fase migracao-mysql

Documentos afetados: PLAN.md (nova tarefa `ajustes/logo-do-painel-aponta-para-supabase`)
