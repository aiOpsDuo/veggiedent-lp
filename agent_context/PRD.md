...
- **Painel de edição por seção** — a página é apresentada no painel como a lista de seções que ela realmente tem hoje, na mesma ordem em que aparecem no site: Hero, Educação, Rotina de Cuidado, Produto, Demonstração, Prova de Autoridade, Captura de Lead, Onde Comprar, FAQ e Footer. **Header e Ingredientes não são editáveis pelo painel** — decisão do usuário em 2026-09-04 (ver `agent_context/CHANGELOG.md`): Ingredientes foi removida do projeto por nunca ter tido conteúdo além do título, e o Header voltou a ser fixo em código.
