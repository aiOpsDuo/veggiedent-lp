...
- Status: **item 2 concluído e ACEITO; item 1 sem defeito reproduzido, com ressalva aberta.** Em 2026-09-04, branch `feat/T27-corrigir-desligar-secao` (a partir do HEAD de `feat/T26-remove-rdstation`, 3 commits, sem merge em `main`). Verificação do orquestrador, rodada por conta própria: `npm run typecheck` limpo nos 4 workspaces; `npm run test` — **662 testes** (302 API + 198 painel + 39 LP + 123 content-schema); `npm run build` limpo (avisos pré-existentes do `zod`, sem relação com a mudança).
